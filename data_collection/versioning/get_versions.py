    ...
    for pattern in MAP_REPO_TO_VERSION_PATTERNS[instance["repo"]]:
        ...
        if matches is not None:
            ...
            if instance['repo'] == 'pyvista/pyvista':
                ...
                return text
            if instance['repo'] == 'tqdm/tqdm':
                text = matches.group(0)

                text = text.split('=')[-1].strip() if '=' in text else text.strip()
                version_item = [v_item.strip() for v_item in text.split(',')]
                text = '.'.join(version_item)
                print(text)
                return text
...
def get_version(instance, is_build=False, path_repo=None):
    ...
    version = None
    if instance['repo']=='dateutil/dateutil' and (instance['pull_number']>=510 and instance['pull_number']<=624 ):
        return '2.7'
    if instance['repo']=='pyca/cryptography' and (instance['pull_number']==8390 ):
        return '40.0'
